...
def _find_repo_root(start: Path) -> Path:
    for parent in [start] + list(start.parents):
        if (parent / "packages" / "manuals").is_dir():
            return parent
        if (parent / "manuals").is_dir():  # legacy layout
            return parent
    # Fallback: assume apps/wind_rag_webapp/backend/ (repo root is 3 levels up)
    try:
...
# Ensure pipeline packages are importable (Agents/, manuals/, Scada/, Faulty_Image_Describtion/)
if PACKAGES_DIR.is_dir():
    sys.path.insert(0, str(PACKAGES_DIR))
else:
    sys.path.insert(0, str(REPO_ROOT))

MANUALS_DIR = Path(os.environ.get("MANUALS_DIR", str(PACKAGES_DIR / "manuals")))
if not MANUALS_DIR.is_dir():
    legacy_manuals = REPO_ROOT / "manuals"
    if legacy_manuals.is_dir():
        MANUALS_DIR = legacy_manuals
RAG_INDEX_DIR = Path(
    os.environ.get("RAG_INDEX_DIR", str(MANUALS_DIR / "manuals_index"))
)

SCADA_DIR = Path(os.environ.get("SCADA_DIR", str(PACKAGES_DIR / "Scada")))
if not SCADA_DIR.is_dir():
    legacy_scada = REPO_ROOT / "Scada"
    if legacy_scada.is_dir():
        SCADA_DIR = legacy_scada

# SCADA cards directories
SCADA_CARDS_DIR = SCADA_DIR / "scada_cards_out"
SCADA_WINDOWS_META_DIR = SCADA_DIR / "scada_windows_meta"
SCADA_BY_CLASS_PATH = SCADA_WINDOWS_META_DIR / "scada_windows_by_class.json"

FAULTY_DIR = Path(
    os.environ.get("FAULTY_DIR", str(PACKAGES_DIR / "Faulty_Image_Describtion"))
)
if not FAULTY_DIR.is_dir():
    legacy_faulty = REPO_ROOT / "Faulty_Image_Describtion"
    if legacy_faulty.is_dir():
        FAULTY_DIR = legacy_faulty

EVAL_DIR = Path(os.environ.get("EVAL_DIR", str(REPO_ROOT / "experiments" / "evalution")))
if not EVAL_DIR.is_dir():
    legacy_eval = REPO_ROOT / "evalution"
    if legacy_eval.is_dir():
        EVAL_DIR = legacy_eval
...
FAULTY_EXAMPLES_PATH = FAULTY_DIR / "examples.json"
