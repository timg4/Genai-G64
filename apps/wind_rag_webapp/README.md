...
- Replace `data/scada_samples.json` with your own predefined time windows.
- Replace `data/chunks.json` with your own chunk export.

## 2) Integrate your real vector DB

In `backend/app.py`, search for `retrieve(`.
- Swap it with your vector search (e.g., embeddings + cosine similarity, FAISS, Qdrant, Pinecone, etc.).
- Keep returning: source, page, page_end, section, chunk_id, score, snippet (plus a short snippet if available).
